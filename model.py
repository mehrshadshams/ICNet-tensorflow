...
class ICNet_BN(Network):
    ...
    def find_tune(self, new_num_class):
        sub12_sum_interp = self.layers['sub12_sum_interp']
        conv5_4_interp = self.layers['conv5_4_interp']
        sub24_sum_interp = self.layers['sub24_sum_interp']

        conv6_cls = self._conv(sub12_sum_interp, 1, 1, new_num_class,
                               1, 1, biased=True, relu=False, name='conv6_cls_2')
        sub4_out = self._conv(conv5_4_interp, 1, 1, new_num_class,
                              1, 1, biased=True, relu=False, name='sub4_out_2')
        sub24_out = self._conv(sub24_sum_interp, 1, 1, new_num_class,
                               1, 1, biased=True, relu=False, name='sub24_out_2')

        return conv6_cls, sub4_out, sub24_out
